...
function buildUserPrompt(context: UnifiedContext, learnedContext: string): string {
...
export class TaskGeneratorAgent {
  async generate(context: UnifiedContext): Promise<AgentTask[]> {
    const cfg = getConfig();

    // Load any learned prompt improvements from semantic memory
    const learnedContext = semanticGet<string>('task_generator.learned_context') ?? '';

    // Build dynamic system prompt (base + any learned additions)
    const learnedSystemAddition = semanticGet<string>('task_generator.system_prompt_addition') ?? '';
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        const tasks: AgentTask[] = JSON.parse(clean);
