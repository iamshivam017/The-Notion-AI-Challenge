...
import { semanticGet } from '../memory/semantic';

interface ScoredTask extends AgentTask {
  score: number;
  scoreBreakdown: Record<string, number>;
}

// ---------------------------------------------------------------------------
// Scoring weights — can be tuned via semantic memory by the self-improver
// ---------------------------------------------------------------------------
interface ScoringWeights {
  priorityBase: Record<AgentTask['priority'], number>;
...
export class PrioritizerAgent {
  private getWeights(): ScoringWeights {
    return semanticGet<ScoringWeights>('prioritizer.weights') ?? DEFAULT_WEIGHTS;
  }

  async rank(tasks: AgentTask[], context: UnifiedContext): Promise<AgentTask[]> {
    ...
    const scored: ScoredTask[] = tasks.map(task => {
      ...
      const touchesStalePage = context.derived.staleTasks.some(
        p => p.id === task.payload?.pageId,
      );
      ...
      const touchesRecentPage = context.notion.recentEdits.some(
        p => p.id === task.payload?.pageId,
      );
      breakdown.recency = touchesRecentPage ? weights.recencyBonus : 0;

      // Overdue bonus
      const touchesOverduePage = context.derived.overdueTasks.some(
        p => p.id === task.payload?.pageId,
      );
      ...
      const touchesBlockedPage = context.derived.blockedItems.some(
        p => p.id === task.payload?.pageId,
      );
    ...
    return scored
      ...
      .map(({ score: _s, scoreBreakdown: _sb, ...task }) => task as AgentTask);
  }
}
